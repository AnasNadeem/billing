...
class UserListDash:
    ...
    def save_user_fun(self):
        ...
        try:
            if self.var_username == '' or self.var_location == '':
                ...
                cur.execute('SELECT * FROM users where username=?', (self.var_username.get(),))
                row_db = cur.fetchone()
                if row_db != None:
    ...
    def del_user_fun(self):
        ...
        try:
            ...
            row_db = cur.fetchone()
            if row_db == None:
                messagebox.showerror('Error', f'Invalid Username9.', parent=self.window)
            else:
                yes_no = messagebox.askyesno('Are you Sure?', f'Sure to Delete {self.var_username.get()} ?', parent=self.window)
                if yes_no:
                    cur.execute("DELETE FROM users where username=?",(self.var_username.get(),))
                    con.commit()
                    self.show_user_fun()
                else:
    ...
    def search_user_fun(self):
        ...
        try:
            if self.var_search_by.get()=='Select By':
                ...
                cur.execute('SELECT * FROM users WHERE username=?', (self.var_search_by_val.get(),))
                rows_db = cur.fetchall()
                if rows_db!=[]:
                    self.main_list_tree.delete(*self.main_list_tree.get_children())
                    for row in rows_db:
                ...
                cur.execute('SELECT * FROM users WHERE admin=?', (self.var_search_by.get(),))
                rows_db = cur.fetchall()
                if rows_db!=[]:
                    self.main_list_tree.delete(*self.main_list_tree.get_children())
                    for row in rows_db:
                ...
                cur.execute('SELECT * FROM users WHERE email=?', (self.var_search_by_val.get(),))
                rows_db = cur.fetchall()
                if rows_db!=[]:
                    self.main_list_tree.delete(*self.main_list_tree.get_children())
                    for row in rows_db:
                ...
                cur.execute('SELECT * FROM users WHERE phone=?', (self.var_search_by_val.get(),))
                rows_db = cur.fetchall()
                if rows_db!=[]:
                    self.main_list_tree.delete(*self.main_list_tree.get_children())
                    for row in rows_db:
                ...
                cur.execute('SELECT * FROM users WHERE location=?', (self.var_search_by_val.get(),))
                rows_db = cur.fetchall()
                if rows_db!=[]:
                    self.main_list_tree.delete(*self.main_list_tree.get_children())
                    for row in rows_db:
