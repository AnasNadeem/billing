...
class BillDash:
    ...
    def show_search_prod_func(self):
        ...
        try:
            if self.var_search_prod_by.get()=='Select By':
                ...
                cur.execute('SELECT * FROM inventory WHERE ven_name=%s', (self.var_search_prd_text.get(),))
                rows_db = cur.fetchall()
                if rows_db!=[]:
                    self.main_list_tree.delete(*self.main_list_tree.get_children())
                    for row in rows_db:
    ...
    def add_to_cart_func(self):
        if self.var_pr_name.get()=='':
            ...
            cur = con.cursor()
            try:
                cur.execute('SELECT * FROM inventory where pr_name=%s and sell_price=%s',(self.var_pr_name.get(),self.var_pr_price.get()))
                row_db = cur.fetchone()
    ...
    def general_bill_func(self):
        ...
        if child_rows==():
            ...
            try:
                ...
                cur.execute("""INSERT INTO bill (cus_name, cus_num, bill_file, date, location)
                    VALUES(%s,%s,%s,%s,%s)""",(
    ...
    def update_bill_and_show(self, cus_name,cus_num,date_bill):
        ...
        cur = con.cursor()
        try:
            cur.execute("""SELECT * from bill where cus_name=%s and date=%s""",(cus_name, date_bill))
            ...
            cur.execute("""UPDATE bill SET bill_file=%s WHERE cus_name=%s and date=%s""",(bill_file,cus_name,date_bill))
            con.commit()
        except Exception as ex:
            messagebox.showerror('Error', f'Error due to {str(ex)}', parent=self.window)
    ...
    def upd_prod_fun(self):
        ...
        try:
            ...
            for rows in child_rows:
                row = self.add_to_cart_tree.item(rows)['values']
                cur.execute("""SELECT * FROM inventory where pr_name=%s AND sell_price=%s""",(row[1],row[2]))
                fetch_data = cur.fetchone()
                cur.execute("""UPDATE inventory SET stocks=%s WHERE pr_name = %s AND sell_price=%s""",(
                    fetch_data[2]-row[3],
                    row[1],
                    row[2]
                ))
