    ...
    def __init__(self, window, username):
        ...
        if self.check_location():
            self.main_list_tree = ttk.Treeview(self.frame_for_tree,
                    columns=("pr_id","pr_name","stocks","cost_price","sell_price","gst","ven_name", "ven_num", "location"), show='headings', yscrollcommand=self.scrolly.set, xscrollcommand=self.scrollx.set )
            ...
            self.main_list_tree.bind("<ButtonRelease-1>", self.get_adm_data_fun)
            self.show_prod_fun()
        else:
            self.main_list_tree = ttk.Treeview(self.frame_for_tree,
        ...
        if self.check_location():
            # ============================Location====================================
            self.location_label = Label(self.frame_for_update, text="Location: ", bg="white", fg="#4f4e4d",
                                        font=("yu gothic ui", 13, "bold"))
            self.location_label.grid(row=2,column=2,padx=40,pady=20)

            self.location_entry = Entry(self.frame_for_update, relief=SUNKEN, bg="white", fg="#6b6a69",
                                        font=("yu gothic ui semibold", 12),textvariable=self.var_location)
            self.location_entry.grid(row=2,column=3,padx=0,pady=20)
        
            # Product Id
            self.prod_id_label = Label(self.frame_for_update, text="Pr ID: ", bg="white", fg="#4f4e4d",
                                        font=("yu gothic ui", 13, "bold"))
            # self.prod_id_label.grid(row=2,column=2,padx=40,pady=20)
            self.prod_id_label.place(x=840, y=160)

            self.prod_id_dyn = Label(self.frame_for_update,textvariable=self.var_pr_id,fg="#4f4e4d",
                                        font=("Roboto Regular", 14, "bold"))
            
            self.prod_id_dyn.place(x=900, y=160)
            ...
            self.prod_id_dyn.place(x=700, y=160)
    ...
    def show_prod_fun(self):
        ...
        try:
            if self.check_location():
                ...
                cur.execute('SELECT * FROM inventory where location=%s',location_row[5])
                rows_db = cur.fetchall()
                self.main_list_tree.delete(*self.main_list_tree.get_children())
                for row in rows_db:
                    self.main_list_tree.insert('', END, values=row)
    ...
    def upd_prod_fun(self):
        ...
        try:
            if self.var_pr_id.get() =='0' or self.var_pr_name.get() == '':
                ...
                cur.execute('SELECT * FROM inventory where pr_id=%s', (self.var_pr_id.get(),))
                row_db = cur.fetchone()
                if row_db == None:
    ...
    def del_prod_fun(self):
        ...
        try:
            ...
            if row_db == None:
                ...
                if yes_no:
                    cur.execute("DELETE FROM inventory where pr_id=%s",(self.var_pr_id.get(),))
                    con.commit()
                    self.show_prod_fun()
                else:
        ...
        try:
            ...
            if fetch_row[5]!='All':
                if self.var_search_by.get()=='Select By':
                    ...
                    cur.execute('SELECT * FROM inventory WHERE pr_id=%s and location=%s', (self.var_search_by_val.get(),fetch_row[5]))
                    rows_db = cur.fetchall()
                    if rows_db!=[]:
                        self.main_list_tree.delete(*self.main_list_tree.get_children())
                        for row in rows_db:
                    ...
                    cur.execute('SELECT * FROM inventory WHERE pr_name=%s and location=%s', (self.var_search_by_val.get().capitalize(),fetch_row[5]))
                    rows_db = cur.fetchall()
                    if rows_db!=[]:
                        self.main_list_tree.delete(*self.main_list_tree.get_children())
                        for row in rows_db:
                    ...
                    cur.execute('SELECT * FROM inventory WHERE ven_name=%s and location=%s', (self.var_search_by_val.get().capitalize(),fetch_row[5]))
                    rows_db = cur.fetchall()
                    if rows_db!=[]:
                        self.main_list_tree.delete(*self.main_list_tree.get_children())
                        for row in rows_db:
                    ...
                    cur.execute('SELECT * FROM inventory WHERE ven_num=%s and location=%s', (self.var_search_by_val.get(),fetch_row[5]))
                    rows_db = cur.fetchall()
                    if rows_db!=[]:
                        self.main_list_tree.delete(*self.main_list_tree.get_children())
                        for row in rows_db:
                ...
                if self.var_search_by.get()=='Select By':
                    ...
                    cur.execute('SELECT * FROM inventory WHERE pr_id=%s', (self.var_search_by_val.get(),))
                    rows_db = cur.fetchall()
                    if rows_db!=[]:
                        self.main_list_tree.delete(*self.main_list_tree.get_children())
                        for row in rows_db:
